...
class UpdateStore(private val directory: File, private val channel: ReleaseChannel, private val packageName: String) {
    ...
    data class Saved(val bundle: UpdateBundle, val artifactId: Long? = null, val etag: String? = null, val route: String = UpdateRoute.GITHUB.key)
    ...
    fun read(name: String): Saved? {
        ...
        val route = root.optString("route", UpdateRoute.GITHUB.key).also { require(UpdateRoute.entries.any { route -> route.key == it }) }
        return Saved(bundle, id, etag, route)
    }
    fun write(name: String, saved: Saved) {
        val bytes = JSONObject().put("schemaVersion", 1).put("release", saved.bundle.releaseJson).put("manifest", saved.bundle.manifestJson)
            .put("assetId", saved.artifactId ?: JSONObject.NULL).put("etag", saved.etag ?: JSONObject.NULL).put("route", saved.route).toString().toByteArray()
