...
@Composable fun PicacgLoginScreen(controller: PasswordAccountController, networkReady: Boolean, showAvatarFrame: Boolean = true, openNetwork: () -> Unit) {
    ...
    Column(Modifier.fillMaxSize().verticalScroll(rememberScrollState()).padding(24.dp), verticalArrangement = Arrangement.spacedBy(16.dp)) {
        ...
        Text("使用${controller.title}账号登录。密码仅用于本次登录；验证成功后加密保存会话。", style = MaterialTheme.typography.bodyMedium)
        ...
        Button(onClick = { val secret = password.toCharArray(); password = ""; controller.login(email, secret) },
        ...
        TextButton(onClick = { password = ""; controller.logout() }, enabled = !operation.busy, modifier = Modifier.fillMaxWidth()) { Text("清除本地会话") }
